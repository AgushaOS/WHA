                           ...
                           size_t write_batch_frames = 8192)
{
    ...
    PowCodec pow_dec((int)num_channels, 0.85f, false, target_kbps / float(num_channels));
    OverlapAddState ola;
    ...
    std::vector<uint8_t> mode_ms, active0, active1;
    std::vector<std::array<float, 4>> is_r;
    std::vector<bool> is_inv, is_use_segmented;
    ...
    for (uint32_t bi = 0; bi < block_count; ++bi) {
        ...
        if (pred_enabled && pred_state.ready && bi > pred_state.last_bi) {
            ...
            if (age <= SAME_MODE_PRED_MAX_AGE) can_state_pred = true;
        }
        
        window.clear(); window_sq.clear();
        ...
        for (int i = 0; i < expected_band_count; ++i) {
            uint8_t byte = blk[ptr + (i >> 3)]; mode_ms[i] = (byte >> (i & 7)) & 1;
        }
        ptr += mode_bytes_loc;
        ...
        for (int i = 0; i < expected_band_count; ++i) {
            uint8_t byte = blk[ptr + (i >> 3)]; active0[i] = (byte >> (i & 7)) & 1;
        }
        ptr += mask_bytes;
        ...
        if (stereo) {
            ...
            for (int i = 0; i < expected_band_count; ++i) {
                uint8_t byte = blk[ptr + (i >> 3)]; active1[i] = (byte >> (i & 7)) & 1;
            }
            ptr += mask_bytes;
        }
        ...
        is_inv.resize(expected_band_count, false);
        is_use_segmented.resize(expected_band_count, false);
        bool is_used = false;
        ...
        if (block_format_version >= 18 && stereo && target_kbps < 510.0f) {
            ...
            if (is_start < expected_band_count) {
                ...
                if (block_format_version >= 20) {
                    ...
                    for (int i = is_start; i < expected_band_count; ++i) if (mode_ms[i]) is_indices.push_back(i);
                    ...
                    if (ptr + total_bytes + inv_bytes <= blk.size()) {
                        if (!is_indices.empty()) {
                            ...
                            auto inv_tmp = unpack_bits(blk.data() + ptr, inv_bytes, (int)is_indices.size());
                            for (size_t j = 0; j < is_indices.size(); ++j) is_inv[is_indices[j]] = inv_tmp[j];
                            ptr += inv_bytes;
                        }
                    }
                } else {
                    ...
                    if (ptr + total_bytes + inv_bytes <= blk.size()) {
                        ...
                        auto inv_tmp = unpack_bits(blk.data() + ptr, inv_bytes, expected_band_count - is_start);
                        for (int i = is_start; i < expected_band_count; ++i) is_inv[i] = inv_tmp[i - is_start];
                        ptr += inv_bytes;
                    }
                }
            }
        ...
        SBRDecodeResult sbr = sbr_decode(blk.data(), ptr, blk.size(), active0, sbr_end, expected_band_count, block_format_version, can_state_pred, pred_state);
        
        steps0.resize(expected_band_count, 0.0f);
        steps1.resize(expected_band_count, 0.0f);
        
        auto decode_scales = [&](std::vector<float>& steps, const std::vector<uint8_t>& active, int k_scale, std::vector<uint32_t>& prev_scale, const std::vector<uint8_t>& prev_active) {
            for (int i = 0; i < expected_band_count; ++i) {
        ...
        for (int i = 0; i < expected_band_count; ++i) {
            ...
            else { ch0_bands[i].clear(); ch0_bands[i].assign(band_shapes[i], 0.0f); }
            
            if (stereo) {
                if (active1[i]) dequantize_band(quant1[i], steps1[i], ch1_bands[i]);
                else { ch1_bands[i].clear(); ch1_bands[i].assign(band_shapes[i], 0.0f); }
            }
        }
            ...
            for (int i = 0; i < expected_band_count; ++i) {
                if (mode_ms[i]) {
                    if (block_format_version >= 18 && is_used && i >= is_start && is_start < expected_band_count) {
                        ...
                        ch0_bands[i] = std::move(left); ch1_bands[i] = std::move(right);
                    } else {
                        std::vector<float> left_tmp, right_tmp;
                        ms_to_lr(ch0_bands[i], ch1_bands[i], left_tmp, right_tmp);
                        ch0_bands[i] = std::move(left_tmp); ch1_bands[i] = std::move(right_tmp);
                    }
                } else {
                    if (block_format_version >= 18 && is_used && i >= is_start && is_start < expected_band_count) {
                        std::vector<float> left_tmp, right_tmp;
        ...
        if (pred_enabled) {
            ...
            for (int i = 0; i < expected_band_count; ++i) if (active0[i]) pred_state.prev_active0[i] = 1;
            
            if (stereo) {
                pred_state.prev_active1.assign(expected_band_count, 0);
                for (int i = 0; i < expected_band_count; ++i) if (active1[i]) pred_state.prev_active1[i] = 1;
            } else {
                pred_state.prev_active1.assign(expected_band_count, 0);
            }
            ...
            pred_state.ready = true; pred_state.last_bi = bi;
        }

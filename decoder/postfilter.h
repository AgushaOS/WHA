...
public:
    ...
    explicit SpectralFloorPostFX(const Config& cfg) : c(cfg) {
        ...
        window.resize(c.nfft);
        for (int i = 0; i < c.nfft; i++) {
            window[i] = 0.5f * (1.f - cosf(2.f * (float)M_PI * i / (c.nfft - 1)));
        }
        
        acc.assign(c.nfft, 0.f);
        prev_gain.assign(nfreq, 1.f);
        noise_memory.assign(nfreq, 0.f);
        floor_cur.assign(nfreq, 0.f); 
        prev_floor.assign(nfreq, 0.f);
        has_prev_floor = false;
        
        s_power.resize(nfreq); 
        s_contrast.resize(nfreq); 
        s_gaindb.resize(nfreq);
        s_valley.resize(nfreq); 
        s_smooth.resize(nfreq); 
        s_gain.resize(nfreq);
...
private:
    ...
    fftwf_complex* freq_mod = nullptr;
    ...
    void processFrame(const float* frame_in, float* frame_out) {
        ...
        if (fast_path) {
            ...
            for (int i = 1; i < NF; i++) {
                ...
                if (p0 == 0) { p0 = -1; need_recalc = true; } 
                else if (p0 > 0) p0--;
                
                if (p1 == 0) { p1 = -1; need_recalc = true; } 
                else if (p1 > 0) p1--;
                
                if (p2 == 0) { p2 = -1; need_recalc = true; } 
                else if (p2 > 0) p2--;
...
public:
    ...
    int process(const float* interleaved, int n_frames) {
        ...
        if ((int)ch_in.size() < n_frames) 
            ch_in.resize(n_frames);
        ...
        for (int ch = 0; ch < num_channels; ch++) {
            ...
            if ((int)per_ch_out[ch].size() < cap) 
                per_ch_out[ch].resize(cap);
